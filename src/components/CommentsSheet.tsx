...
const CommentsSheet = ({ entryId, onClose }: Props) => {
  ...
  const handlePost = async () => {
    ...
    setPosting(true);
    await postComment(user.id, entryId, body.trim());
    setBody('');
    await load();
    setPosting(false);
  };
      ...
      <div className="relative bg-card rounded-t-3xl flex flex-col" style={{ maxHeight: '75vh', paddingBottom: 'env(safe-area-inset-bottom)' }}>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {comments.map(c => (
            <div key={c.id} className="flex gap-3">
              ...
              {c.user_id === user?.id && (
                <button onClick={() => handleDelete(c.id)} className="text-muted-foreground hover:text-red transition-colors flex-shrink-0">
                  <Trash2 size={14} />
                </button>
              )}
            </div>
        ...
        <div className="px-5 py-3 border-t border-border flex gap-3 items-center">
          ...
          <button
            ...
            className="w-10 h-10 rounded-xl bg-clean flex items-center justify-center disabled:opacity-40 transition-all active:scale-95"
          >
